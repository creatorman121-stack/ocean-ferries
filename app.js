...
const APP_VERSION = 'V96 Vessel Source Corrected Edition';
...
function dashboard(){
  ...
  $('content').innerHTML = hero('OceanJet PortDesk','Online-ready operations console with automatic scheduled fares, advisory updates, and offline fallback.','V96 Vessel Source Corrected') + `
    <div class="grid">
...
function downloadScheduleCSV(){ download('OceanJet_V96_Schedule_'+Date.now()+'.csv', buildScheduleCSV(), 'text/csv'); }
...
function exportJSON(){ download('OceanJet_V96_Backup_'+Date.now()+'.json', JSON.stringify({version:APP_VERSION, exportedAt:new Date().toISOString(), state}, null, 2), 'application/json'); }
...
function downloadSyncPayload(){ download('OceanJet_V96_Sync_Payload_'+Date.now()+'.json', JSON.stringify(syncPayload(), null, 2), 'application/json'); }
